...
const Color GRID_LINE_COLOR = blue;
...
void drawGrid() {
    ...
    for(int row = 0; row <= ROW_NB; row++){
        DrawLine(startPosX,row*CELL_HEIGHT,BOARD_WIDTH, row*CELL_HEIGHT,GRID_LINE_COLOR);
    }
    for(int col = 0; col <= COL_NB; col++){
        DrawLine(col *CELL_WIDTH,startPosY,col*CELL_WIDTH, BOARD_HEIGHT, GRID_LINE_COLOR);
    }
}
...
void gameOver(){
     const char *msg = "GAME OVER";
            ...
            DrawText(msg, BOARD_WIDTH + (INF_AREA/2) - textW/2, BOARD_HEIGHT/2 - fontSize, fontSize, RAYWHITE);
            const char *msg2 = "Press R to restart";
            int fs2 = 14;
            DrawText(msg2, BOARD_WIDTH + (INF_AREA/2) - MeasureText(msg2, fs2)/2, BOARD_HEIGHT/2 + 10, fs2, RAYWHITE);
...
// Lock active block into the field (simple implementation)
void lockActiveBlock(const ActiveBlock &activeBlock){
    for(int x = 0; x < 4; x++){
        for(int y = 0; y < 4; y++){
            int index = Rotate(x, y, activeBlock.orientation);
...
bool DoesPieceFit( const ActiveBlock &activeBlock)
{
    ...
    for (int px = 0; px < 4; px++)
        for (int py = 0; py < 4; py++)
        {
            ...
            {
                if (tetromino[activeBlock.block][index] == 'X') return false;
            }
        }
    return true;
}
    ...
    for(int x= 0; x < 4; x++){
        for(int y = 0; y < 4; y++){
            ...
            if(tetromino[activeBlock.block][index] == 'X'){
                ...
                if(field[newY * COL_NB + newX] != -1){ // Collision with a locked block.
                    return false;
                }
            }
        }
